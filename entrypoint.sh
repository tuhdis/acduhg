...
if [[ -z "${Vless_Path}" ]]; then
  Vless_Path="/budheigldsg"
fi
echo ${Vless_Path}

if [[ -z "${Vless_UUID}" ]]; then
  Vless_UUID="71fb5524-a63d-48d1-bc45-f768e9b17eed"
fi
echo ${Vless_UUID}

if [[ -z "${Vmess_Path}" ]]; then
  Vmess_Path="/ndokfgoisof"
fi
echo ${Vmess_Path}

if [[ -z "${Vmess_UUID}" ]]; then
  Vmess_UUID="c61cdca4-261b-40f2-b417-00979b84e833"
fi
echo ${Vmess_UUID}

if [[ -z "${Share_Path}" ]]; then
  Share_Path="/udjogjoafgj"
fi
echo ${Share_Path}
